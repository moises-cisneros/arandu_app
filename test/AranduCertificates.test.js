...
describe("AranduCertificates", function () {
    ...
    describe("Minting", function () {
        it("Should allow the owner to mint certificates", async function () {
            await expect(aranduCertificates.safeMint(student1.address, "CertificateURI")).to.not.be.reverted;
        ...
        it("Should fail if a non-owner tries to mint", async function () {
            await expect(aranduCertificates.connect(teacher1).safeMint(student1.address, "CertificateURI")).to.be.revertedWithCustomError(aranduCertificates, "OwnableUnauthorizedAccount");
        });
    });

    describe("Transfer Prevention", function () {
        it("Should prevent transfers of certificates", async function () {
            await aranduCertificates.safeMint(student1.address, "CertificateURI");
